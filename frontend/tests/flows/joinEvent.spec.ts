...
test('Sumarse evento con usuario logueado', async ({ page }) => {
  ...
  await page.getByRole('option', { name: 'Fútbol 5' }).click();
  ...
  await expect(page.locator('#root')).toContainText('Club: El Predio Ciudad');
  await page.getByRole('heading', { name: 'Quedan 9 lugares' }).click();
...
test('Sumarse a evento con usuario no logueado', async ({ page }) => {
  ...
  await page.getByRole('option', { name: 'Fútbol 7' }).click();
  await page.getByLabel('Open').nth(1).click();
  await page.getByRole('option', { name: 'Nuñez' }).click();
  await page.getByLabel('Choose date').click();
  await page.getByRole('gridcell', { name: '15', exact: true }).click();
  await page.getByRole('button', { name: 'Buscar' }).click();
  await expect(page.locator('.MuiGrid-root > .MuiPaper-root').first()).toBeVisible();
  await page.getByRole('button', { name: 'Ver Más' }).first().click();
  await expect(page.getByRole('button', { name: '¡Me Sumo!' })).toBeVisible();
  await page.getByRole('button', { name: '¡Me Sumo!' }).click();
  await expect(page.getByRole('dialog')).toContainText('¡Hola!Tenés que estar logueado para sumarte al evento!¡OK!');
  await page.getByRole('button', { name: '¡OK!' }).click();
});